...
Корутины — это функции, которые можно приостанавливать

В отличие от обычной функции, корутина:
- не выполняется сразу, а возвращает объект coroutine;
- может быть приостановлена в любой момент (оператор await или yield);
- затем возобновлена.
"""
...
def task1_fsm(): # Конечный автомат
    ...
    while True: # основной цикл
        ...
        # yield соответствует приёму входного символа автомата, (Корутины позволяют естественным образом описывать автоматы без внешних таблиц переходов)
        # Состояние хранится локально, переходы описываются явно
        choice = yield # корутина остановилась и ждёт входных данных, когда данные приходят, yield возвращает их
